      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 p-3.5 bg-zinc-950/70 border border-zinc-800 rounded-xl">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <div className="flex items-center gap-1.5">
              ...
              {onUpdateStatus && statuses.length > 0 ? (
                <select
                  ...
                  className="px-2.5 py-1 rounded bg-zinc-900 border border-zinc-700 text-xs text-zinc-200 capitalize focus:outline-none focus:border-indigo-500"
                >
                  {statuses.map(st => (
            ...
            {sprint && (
              <div className="flex items-center gap-1 text-xs text-amber-300 bg-amber-950/30 px-2 py-0.5 rounded border border-amber-800/30">
            ...
            {release && (
              <div className="flex items-center gap-1 text-xs text-indigo-300 bg-indigo-950/30 px-2 py-0.5 rounded border border-indigo-800/30">
        ...
        <div className="space-y-1.5">
          ...
          <div className="p-3.5 rounded-xl bg-zinc-900/60 border border-zinc-800 text-xs text-zinc-300 whitespace-pre-wrap leading-relaxed">
          ...
          {isEditingContext ? (
            <div className="space-y-2">
              <textarea
                ...
                className="w-full p-3 rounded-lg bg-zinc-950 border border-zinc-800 focus:border-purple-500 focus:outline-none font-mono text-xs text-zinc-200"
              />
              <div className="flex justify-end">
                <button
                  ...
                  className="px-3 py-1.5 rounded-lg bg-purple-600 hover:bg-purple-500 text-white text-xs font-medium transition disabled:opacity-50"
                >
            ...
            <div className="p-3 rounded-xl bg-purple-950/10 border border-purple-900/30 text-xs font-mono text-purple-200 whitespace-pre-wrap leading-relaxed">
              {item.context.t}
            </div>
          ) : (
            <div className="text-xs text-zinc-500 italic p-3 bg-zinc-950/40 rounded-lg border border-zinc-900">
        ...
        <div className="space-y-3 border-t border-zinc-800/80 pt-4">
          ...
          {item.progress && item.progress.length > 0 ? (
            <div className="space-y-2.5 max-h-48 overflow-y-auto pr-1">
              {item.progress.map((p, idx) => (
                <div
                  key={p.id || idx}
                  className="p-3 rounded-lg bg-zinc-950/60 border border-zinc-800/80 text-xs flex flex-col sm:flex-row sm:items-start justify-between gap-2"
                >
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      <StatusBadge status={p.status} />
                      {p.proof && (
                        <span className="font-mono text-[10px] bg-zinc-800 text-emerald-400 px-1.5 py-0.5 rounded border border-emerald-900/40">
                          {p.proof}
                        </span>
                      )}
                    </div>
                    <p className="text-zinc-300">{p.t}</p>
                  </div>

                  <div className="text-[10px] text-zinc-500 whitespace-nowrap self-start sm:self-center">
                    {p.user && <span className="mr-2">by {p.user}</span>}
          ...
          {/* Add Progress form */}
          {onAddProgress && (
            <form onSubmit={handleAddProgress} className="space-y-2 pt-2">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                <input
                  ...
                  className="sm:col-span-2 px-3 py-1.5 rounded-lg bg-zinc-950 border border-zinc-800 text-xs text-zinc-200 focus:outline-none focus:border-indigo-500"
                />
                <input
                  type="text"
                  placeholder="Git SHA / Proof (optional)"
                  value={progressProof}
                  onChange={e => setProgressProof(e.target.value)}
                  className="px-3 py-1.5 rounded-lg bg-zinc-950 border border-zinc-800 text-xs font-mono text-zinc-200 focus:outline-none focus:border-indigo-500"
                />
              </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="px-3 py-1.5 rounded-lg bg-zinc-800 hover:bg-zinc-700 text-zinc-200 text-xs font-medium flex items-center gap-1 transition disabled:opacity-50"
                >

...
export function ProjectCard({ project, onClick }: ProjectCardProps) {
  return (
    <div
      ...
      className="group bg-zinc-900/60 border border-zinc-800/80 hover:border-indigo-500/50 rounded-2xl p-6 transition-all duration-200 cursor-pointer shadow-sm hover:shadow-xl hover:shadow-indigo-500/5 flex flex-col justify-between"
    ...
    >
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-zinc-800/60 border border-zinc-700/50 text-xs text-zinc-300">
      ...
      <div className="pt-5 mt-4 border-t border-zinc-800/60 flex items-center justify-between text-xs text-zinc-500">
        <div className="flex items-center gap-1.5 overflow-hidden">
          {project.statuses?.slice(0, 4).map(st => (
            <span
              key={st.id}
              className="px-2 py-0.5 rounded text-[10px] font-medium bg-zinc-800/80 text-zinc-400 capitalize"
            >
              {st.name}
            </span>
          ))}
        ...
        <div className="flex items-center gap-1 text-zinc-400 group-hover:text-indigo-400 font-medium transition">
          <span>Open</span>
